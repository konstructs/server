...
class WorldSpec extends WordSpec with Matchers {
  ...
  "A Position" should {
    "return chunk 0, 0 for 0, 0, 0" in {
      Position(0, 0, 0).chunk shouldEqual Chunk(0, 0, 0)
    }

    "return chunk 0, 1 for 0, 0, 32" in {
      Position(0, 0, 32).chunk shouldEqual Chunk(0, 1, 0)
    }

    "return chunk 1, 0 for 32, 0, 0" in {
      Position(32, 0, 0).chunk shouldEqual Chunk(1, 0, 0)
    }

    "return chunk 1, 1 for 32, 0, 32" in {
      Position(32, 0, 32).chunk shouldEqual Chunk(1, 1, 0)
    }

    "return chunk 0, 0 for 31, 0, 31" in {
      Position(31, 0, 31).chunk shouldEqual Chunk(0, 0, 0)
    }

    "return chunk 0, -1 for 0, 0, -1" in {
      Position(0, 0, -1).chunk shouldEqual Chunk(0, -1, 0)
    }

    "return chunk -1, -1 for -1, 0, -1" in {
      Position(-1, 0, -1).chunk shouldEqual Chunk(-1, -1, 0)
    }
    ...
    "return chunk -2, -2 for -33, 0, -33" in {
      Position(-33, 0, -33).chunk shouldEqual Chunk(-2, -2, 0)
    }

    "return chunk -1, 0 for -1, 0, 0" in {
      Position(-1, 0, 0).chunk shouldEqual Chunk(-1, 0, 0)
    }
