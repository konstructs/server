...
case class BlockFactoryImpl(blockTypeIdMapping: java.util.Map[BlockTypeId, Integer],
                            ...
                            blockTypes: java.util.Map[BlockTypeId, BlockType])
...
object BlockFactoryImpl {
  ...
  private def addBlockType(
      blockTypeIdMapping: java.util.Map[BlockTypeId, Integer],
      wMapping: java.util.Map[Integer, BlockTypeId],
  ...
  def apply(defined: java.util.Map[Integer, BlockTypeId],
            configured: Seq[(BlockTypeId, BlockType)]): BlockFactoryImpl = {
...
object BlockMetaActor {
  ...
  case class BlockClass(obstacle: Option[Boolean],
                        shape: Option[BlockShape],
                        state: Option[BlockState],
                        durability: Option[Float],
                        damage: Option[Float],
                        damageMultipliers: Map[BlockOrClassId, Float],
                        orientable: Option[Boolean],
                        destroyedAs: Option[BlockTypeId],
                        lightColour: Option[Colour],
                        lightLevel: Option[LightLevel])
  case class BlockDefault(obstacle: Boolean,
                          shape: BlockShape,
                          state: BlockState,
                          durability: Float,
                          damage: Float,
  ...
  def readClasses(config: TypesafeConfig): Array[BlockClassId] =
    if (config.hasPath("classes")) {
      val classConfig = config.getConfig("classes")
      classConfig.root.entrySet.asScala
        .map(e =>
  ...
  def readDamageMultipliers(config: TypesafeConfig): Map[BlockOrClassId, Float] =
    if (config.hasPath("damage-multipliers")) {
      ...
      damageConfig.root.entrySet.asScala
        .map(e =>
  ...
  def readIsObstacle(config: TypesafeConfig): Option[Boolean] =
    if (config.hasPath("obstacle")) {
      Some(config.getBoolean("obstacle"))
    } else {
      None
    }

  def readShape(config: TypesafeConfig): Option[BlockShape] =
  ...
  def readState(config: TypesafeConfig): Option[BlockState] =
  ...
  def readDurability(config: TypesafeConfig): Option[Float] =
    if (config.hasPath("durability")) {
      Some(config.getDouble("durability").toFloat)
    } else {
      None
    }

  def readDamage(config: TypesafeConfig): Option[Float] =
    if (config.hasPath("damage")) {
      Some(config.getDouble("damage").toFloat)
    } else {
      None
    }

  def readOrientable(config: TypesafeConfig): Option[Boolean] =
    if (config.hasPath("orientable")) {
      Some(config.getBoolean("orientable"))
    } else {
      None
    }

  def readDestroyedAs(config: TypesafeConfig): Option[BlockTypeId] =
    if (config.hasPath("destroyed-as")) {
      Some(BlockTypeId.fromString(config.getString("destroyed-as")))
    } else {
      None
    }

  def readLightColour(config: TypesafeConfig): Option[Colour] =
    if (config.hasPath("light-colour")) {
      Some(Colour.fromRgbHexString(config.getString("light-colour")))
    } else {
      None
    }

  def readLightLevel(config: TypesafeConfig): Option[LightLevel] =
    if (config.hasPath("light-level")) {
      Some(LightLevel.get(config.getInt("light-level")))
    } else {
      None
    }

  def blockType(idString: String,
    ...
    val blockType = if (config.hasPath("faces")) {
      ...
      new BlockType(faces.asScala.map(_ + texturePosition).toArray,
                    shape,
                    isObstacle,
                    false,
                    state,
      ...
      new BlockType(
        Array(texturePosition, texturePosition, texturePosition, texturePosition, texturePosition, texturePosition),
        shape,
        isObstacle,
        false,
        state,
  ...
  def isTransparent(texture: BufferedImage): Boolean = {
    for (x <- 0 until texture.getWidth;
  ...
  def parseBlocks(config: TypesafeConfig,
                  classMap: Map[BlockClassId, BlockClass]): (Seq[(BlockTypeId, BlockType)], Array[Byte]) = {
    val blocks = config.root.entrySet.asScala.map { e =>
      e.getKey -> config.getConfig(e.getKey)
    }
    var texturePosition = 0
    val textures = new BufferedImage(NumTextures * TextureSize, NumTextures * TextureSize, BufferedImage.TYPE_INT_RGB)
